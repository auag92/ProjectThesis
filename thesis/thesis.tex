...
%The Partial Differential Equations that consitute our model can be numerically solved 
...
%In the explicit scheme the dependent variable can be easily represented in terms of the known quantities and direct 
%iterative method can be used to solve the time evolution problem. The explicit methods are simple to implement and computationally 
%much less expensive. Another advantage of explicit method is that the iterations correspond to actual timestep. 
%However the explicit scheme tends to become unstable for larger value of time steps ($dt$). 
%Also, as the time step needs to be very small, the computational time goes up. 
